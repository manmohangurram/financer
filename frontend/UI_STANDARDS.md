...
- **Themes:** two themes — `dark` (default) and `light`, switched via `data-theme` and persisted in `localStorage` (`frontend/src/lib/theme.ts`). App tokens (`surface`, `text`, `subtle`, `bg`, `income`/`expense`, …) have light overrides in the `[data-theme='light']` block in `main.css`; daisyUI base colors come from the `light` theme block. `CATEGORY_PALETTE_LIGHT` is used automatically when `theme.current === 'light'` (category badges must pass AA on the 9.4%-tinted badge background).
