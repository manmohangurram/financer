...
- **Env vars:** `FINANCER_DB_PATH` (default `data/financer.db`), `FINANCER_JWT_SECRET`, `FINANCER_ADDR` (`:8080`), `FINANCER_TLS_CERT`/`KEY` (enable HTTP/3 QUIC), `FINANCER_LOG_LEVEL` (`info` default, `debug` adds per-request logs — uses `logx`, a `log/slog` wrapper in `logx/logx.go`; bind request context once via `logx.Request(method, path, user)`), quote/history refresh intervals. Frontend API URL: `VITE_API_URL` (default `http://localhost:8080`).
