//! MCP tool handler: one method per tool, plus the auth context they read.
...
fn parse_type(s: &str) -> Option<TransactionType> {
    TransactionType::from_str(&s.to_ascii_uppercase()).ok()
}
