...
impl TransferRuleService {
    ...
    pub async fn run_rule(&self, user_id: &str, rule_id: &str) -> Result<RunRuleResponse> {
        ...
        let conds: Vec<ConditionData> = rule.conditions.iter().map(|c| ConditionData {
            match_field: c.match_field,
            operator: c.operator,
            pattern: c.pattern.clone(),
        }).collect();

        let rows = self.transactions.list(user_id, &TransactionListFilter { transaction_type: Some(TransactionType::Debit), ..Default::default() }).await?;
        ...
        Ok(RunRuleResponse { matched, linked, created })
    }
