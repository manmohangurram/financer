...
impl CategoryService {
    ...
    pub async fn create(&self, user_id: &str, names: &[String]) -> Result<BulkResult> {
        let inputs: Vec<_> = names
            .iter()
            .filter(|n| !n.is_empty())
            .map(|n| CategoryCreateInput { name: n.clone() })
            .collect();
        match self.repo.create(user_id, &inputs).await {
            Ok(_) => Ok(BulkResult {
                success: true,
                message: "categories created successfully".to_string(),
                failed_ids: Vec::new(),
                skipped: 0,
            }),
            Err(e) => Ok(BulkResult {
                success: false,
                message: "some categories failed".to_string(),
                failed_ids: vec![e.message],
                skipped: 0,
            }),
        }
    }
    ...
    pub async fn update(&self, user_id: &str, inputs: &[(String, String)]) -> Result<BulkResult> {
        let repo_inputs: Vec<_> = inputs
            .iter()
            .map(|(id, name)| CategoryUpdateInput {
                id: id.clone(),
                name: name.clone(),
            })
            .collect();
        ...
        Ok(BulkResult {
            success: true,
            message: "categories updated successfully".to_string(),
            failed_ids: Vec::new(),
            skipped: 0,
        })
    }
    ...
    pub async fn delete(&self, user_id: &str, ids: &[String]) -> Result<BulkResult> {
        ...
        Ok(BulkResult {
            success: true,
            message: "categories deleted successfully".to_string(),
            failed_ids: Vec::new(),
            skipped: 0,
        })
    }
...
fn category_wire(c: &CategoryRow) -> CategoryResponse {
    CategoryResponse {
        id: c.id.clone(),
        name: c.name.clone(),
        created_at: ts_rfc3339(&c.created_at),
    }
}
