...
impl TransferService {
    ...
    pub async fn create_counterpart(
        &self,
        user_id: &str,
        txn_id: &str,
        to_account_id: &str,
    ) -> Result<CreateTransferResp> {
        ...
        if to_account_id.is_empty() || to_account_id == src.account_id {
            return Err(ApiError::bad_request(
                "to account must differ from the source account",
            ));
        }
        if self
            .account_repo
            .get_by_id(user_id, to_account_id)
            .await?
            .is_none()
        {
        ...
        .await?
        {
            ResolveOutcome::Noop => Err(ApiError::bad_request(
                "source transaction is already linked",
            )),
            ResolveOutcome::Linked {
                ...
                if src.transaction_type == TransactionType::Debit {
                    Ok(CreateTransferResp {
                        debit_transaction_id: src.id,
                        credit_transaction_id: counterpart_id,
                    })
                } else {
                    Ok(CreateTransferResp {
                        debit_transaction_id: counterpart_id,
                        credit_transaction_id: src.id,
                    })
                }
            }
        }
    ...
    pub async fn link_transfers(
        ...
        for (i, (debit_id, credit_id)) in links.iter().enumerate() {
            if debit_id.is_empty() || credit_id.is_empty() {
                failed.push(format!(
                    "link {i}: both debit and credit transaction IDs are required"
                ));
                continue;
            }
            if debit_id == credit_id {
                failed.push(format!(
                    "link {i}: debit and credit transactions must be different"
                ));
                continue;
            }
            let Some((dt, _, da)) = self
                .transaction_repo
                .get_by_id_for_transfer(user_id, debit_id)
                .await?
            else {
                failed.push(format!("debit transaction {debit_id} not found"));
                continue;
            };
            let Some((ct, _, ca)) = self
                .transaction_repo
                .get_by_id_for_transfer(user_id, credit_id)
                .await?
            else {
            ...
            if da == ca {
                failed.push(format!(
                    "link {i}: debit and credit must be from different accounts"
                ));
                continue;
            }
            let debit_linked = self
                .transaction_repo
                .is_transaction_linked(debit_id)
                .await?;
            let credit_linked = self
                .transaction_repo
                .is_transaction_linked(credit_id)
                .await?;
            if debit_linked || credit_linked {
                failed.push(format!(
                    "one or both transactions in link {i} are already linked"
                ));
                continue;
            }
        ...
        if !failed.is_empty() {
            return Ok(BulkResult {
                success: false,
                message: "some transfers failed".to_string(),
                failed_ids: failed,
                skipped: 0,
            });
        }
        Ok(BulkResult {
            success: true,
            message: "transfers linked successfully".to_string(),
            failed_ids: Vec::new(),
            skipped: 0,
        })
    }
    ...
    pub async fn unlink_transfers(&self, user_id: &str, ids: &[String]) -> Result<BulkResult> {
        ...
        if !errs.is_empty() {
            return Ok(BulkResult {
                success: false,
                message: "some unlinks failed".to_string(),
                failed_ids: errs,
                skipped: 0,
            });
        }
        Ok(BulkResult {
            success: true,
            message: "transfer links removed successfully".to_string(),
            failed_ids: Vec::new(),
            skipped: 0,
        })
    }
...
pub async fn resolve_transfer(
    ...
    let opposite = if src.transaction_type == TransactionType::Credit {
        ...
        .await?
    {
        ...
        let errs = transaction_repo
            .create_links(user_id, &[(debit_id, credit_id)])
            .await?;
        if let Some(e) = errs.first() {
            return Err(ApiError::internal(e.clone()));
        }
        return Ok(ResolveOutcome::Linked {
            counterpart_id: cand.id,
            created: false,
        });
    }
    ...
    let counter_txn = Transaction {
        ...
        name: format!(
            "Transfer from {}",
            account_display(user_id, &src.account_id, account_repo).await
        ),
        amount: src.amount,
    ...
    account_repo
        .update_balance(
            &counter_txn.account_id,
            if opposite == TransactionType::Credit {
                src.amount
            } else {
                -src.amount
            },
        ...
        .await?;
    let (c, d) = if opposite == TransactionType::Credit {
        (src.amount, 0.0)
    } else {
        (0.0, src.amount)
    };
    account_repo
        .apply_totals(&counter_txn.account_id, c, d)
        .await?;
    ...
    Ok(ResolveOutcome::Linked {
        counterpart_id: counter_txn.id,
        created: true,
    })
}

async fn account_display(
    user_id: &str,
    account_id: &str,
    account_repo: &dyn AccountRepo,
) -> String {
