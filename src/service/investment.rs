...
impl InvestmentService {
    ...
    pub async fn create(
        ...
        if it != InvestmentType::Stock && it != InvestmentType::MutualFund {
            return Err(ApiError::bad_request(
                "INVESTMENT_TYPE must be STOCK or MUTUAL_FUND",
            ));
        }
        ...
        let inst = self
            .repo
            .create_investment(user_id, symbol, name, it, manual_nav)
            .await?;
        ...
        if it == InvestmentType::Stock && !symbol.is_empty() {
            ...
            if let Some(q) = quotes.get(symbol) {
                let _ = self
                    .repo
                    .update_quote(&inst.id, q.price, q.prev_close)
                    .await;
            }
        }
        let mut inst = inst;
    ...
    pub async fn get(&self, user_id: &str, id: &str) -> Result<Investment> {
        let inst = self
            .repo
            .get_investment(user_id, id)
            .await?
            .ok_or_else(|| ApiError::not_found(format!("investment {id} not found")))?;
        self.with_position(user_id, &inst).await
    }
    ...
    pub async fn update(
        ...
        let inst = self
            .repo
            .update_investment(user_id, id, symbol, name, it, manual_nav)
            .await?;
        self.with_position(user_id, &inst).await
    }
    ...
    pub async fn add_lot(
        ...
        let inst = self
            .repo
            .get_investment(user_id, investment_id)
            .await?
            .ok_or_else(|| ApiError::not_found(format!("investment {investment_id} not found")))?;
        if side != 1 && side != -1 {
    ...
    pub async fn update_lot(
        ...
        if !self
            .repo
            .update_lot(user_id, id, round2(quantity), round2(price), occurred_at)
            .await?
        {
            return Err(ApiError::not_found(format!("lot {id} not found")));
        }
        ...
        lots.into_iter()
            .find(|l| l.id == id)
            .map(|l| lot_wire(&l))
            .ok_or_else(|| ApiError::not_found(format!("lot {id} not found")))
    }
    ...
    pub async fn import(&self, user_id: &str, rows: &[ImportRow]) -> Result<(i64, i64)> {
        ...
        for row in rows {
            ...
            let inst = match self.repo.get_by_symbol(user_id, &row.symbol).await? {
                ...
                None => {
                    self.repo
                        .create_investment(user_id, &row.symbol, &row.name, typ, 0.0)
                        .await?
                }
            };
            if row.side == -1 {
    ...
    pub async fn search_symbols(
        &self,
        query: &str,
    ) -> Result<Vec<crate::service::yahoo::SymbolResult>> {
        if query.is_empty() {
            return Ok(Vec::new());
        }
        self.yahoo.search(query).await.map_err(|e| {
            ApiError::bad_gateway(format!("symbol search temporarily unavailable: {e}"))
        })
    }
    ...
    pub async fn refresh_prices(&self, user_id: &str) -> Result<i64> {
        ...
        for (sym, quote) in quotes {
            if let Some(id) = sym_idx.get(&sym) {
                self.repo
                    .update_quote(id, quote.price, quote.prev_close)
                    .await?;
                updated += 1;
            }
        }
    ...
    pub async fn portfolio_summary(&self, user_id: &str) -> Result<PortfolioSummary> {
        ...
        let mut lots_by_inv: std::collections::HashMap<String, Vec<LotRow>> =
            std::collections::HashMap::new();
        for l in all_lots {
            lots_by_inv
                .entry(l.investment_id.clone())
                .or_default()
                .push(l);
        }
    ...
    pub async fn get_price_history(
        ...
        if !force {
            let (ts, closes, last_fetched) = self
                .repo
                .get_price_history(user_id, investment_id, &cache_key)
                .await?;
            let fresh_secs = price_history_freshness(range_id);
            if !ts.is_empty()
                && crate::utils::timex::now_utc().timestamp() - last_fetched < fresh_secs
            {
                let points: Vec<PricePoint> = ts
                    .iter()
                    .zip(closes.iter())
                    .map(|(t, c)| PricePoint { t: *t, close: *c })
                    .collect();
                return Ok(points);
            }
        }
        self.fetch_and_cache(&inst, &cache_key, &cfg, period1, period2)
            .await
    }
    ...
    async fn fetch_and_cache(
        ...
        let raw = self
            .yahoo
            .get_history(&inst.symbol, &cfg.interval, cfg.limit, period1, period2)
            .await
            .map_err(|e| {
                ApiError::bad_gateway(format!("price data temporarily unavailable: {e}"))
            })?;
        ...
        let _ = self
            ...
            .upsert_price_history(
                ...
                &ts,
                &closes,
                crate::utils::timex::now_utc().timestamp(),
            )
            .await;
        Ok(points)
    }
...
fn resolve_range(range_id: &str, from: &str, to: &str) -> Result<(RangeConfig, String, i64, i64)> {
    if !from.is_empty() || !to.is_empty() {
        let f = chrono::NaiveDate::parse_from_str(from, "%Y-%m-%d")
            .map_err(|_| ApiError::bad_request("from and to must be YYYY-MM-DD"))?;
        let t = chrono::NaiveDate::parse_from_str(to, "%Y-%m-%d")
            .map_err(|_| ApiError::bad_request("from and to must be YYYY-MM-DD"))?;
        if t < f || (t - f).num_days() > 5 * 365 {
            return Err(ApiError::bad_request(
                "custom range must be at most 5 years",
            ));
        }
        let period1 = f.and_hms_opt(0, 0, 0).unwrap().and_utc().timestamp();
        let period2 = (t + chrono::Duration::days(1))
            .and_hms_opt(0, 0, 0)
            .unwrap()
            .and_utc()
            .timestamp();
    ...
    let cfg = ranges.get(range_id).cloned().ok_or_else(|| {
        ApiError::bad_request("range must be one of 1d,7d,1m,6m,1y,3y or provide from/to")
    })?;
...
fn preset_1d(now: chrono::DateTime<chrono::Utc>) -> (i64, i64) {
    ...
    if wd == chrono::Weekday::Sat || wd == chrono::Weekday::Sun {
        ...
        return (
            start.timestamp(),
            (start + chrono::Duration::days(1)).timestamp(),
        );
    }
    (
...
fn aggregate_price_points(points: &[PricePoint], agg: &str) -> Vec<PricePoint> {
    ...
    for p in points {
        let t = chrono::DateTime::from_timestamp(p.t, 0)
            .unwrap_or_default()
            .naive_utc();
        let key = if agg == "week" {
