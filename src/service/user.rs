...
impl UserService {
    ...
    pub async fn update_profile(&self, user_id: &str, name: &str, email: &str, avatar_url: &str) -> Result<ProfileResponse> {
        if name.is_empty() && email.is_empty() && avatar_url.is_empty() {
            return Err(ApiError::bad_request("nothing to update"));
        }
        self.repo.update_profile(user_id, name, email, avatar_url).await.map_err(|_| ApiError::conflict("email already in use"))?;
        self.profile(user_id).await
    }

    /// Verify current password, update hash, bump `token_version`, re-issue tokens.
    pub async fn change_password(&self, user_id: &str, current_password: &str, new_password: &str) -> Result<AuthResponse> {
    ...
    pub async fn logout_all(&self, user_id: &str) -> Result<()> {
        self.repo.logout_all(user_id).await.map_err(|_| ApiError::internal("failed to log out all sessions"))
    }
    ...
    pub async fn save_avatar(&self, user_id: &str, data: &[u8]) -> Result<ProfileResponse> {
        ...
        let Some(ext) = infer_avatar_type(data) else {
            return Err(ApiError::bad_request("avatar must be a png, jpeg, or webp image"));
        };

        std::fs::create_dir_all(&self.avatar_dir).map_err(|_| ApiError::internal("failed to save avatar"))?;
        ...
        self.repo.update_profile(user_id, "", "", &avatar_url).await.map_err(|_| ApiError::internal("failed to update avatar"))?;
        self.profile(user_id).await
    }
}
...
mod tests {
    ...
    fn avatar_sniffing() {
        ...
        let webp = [b'R', b'I', b'F', b'F', 0, 0, 0, 0, b'W', b'E', b'B', b'P', 1, 2, 3];
