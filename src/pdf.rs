//! PDF statement import: extract tables from a PDF, treating each table's first
//! row as its column headers (the same shape the CSV/XLSX import path produces).
//!
...
/// A parsed table: `headers` from the first row, `rows` are the data rows.
...
/// Extract the first table found anywhere in `bytes`. The first row becomes the
/// headers; blank header cells are named `column1`, `column2`, ….
/// Extract **every** table in the document, in page order. Each table's first
/// row becomes its headers; blank header cells are named `column1`, `column2`, …
pub fn extract_tables(bytes: &[u8]) -> Result<Vec<ExtractedTable>> {
...
/// Merge every table into one import-ready table: the first table's headers,
/// then every table's data rows.
///
/// A table's first row is only a repeated header when it matches the merged
/// headers (a table continuing on the next page may repeat them). Otherwise the
/// first row is real data — statements often emit one table per page with the
/// header row only on the first — so it is kept as a data row.
pub fn extract_table(bytes: &[u8]) -> Result<ExtractedTable> {
    let mut tables = extract_tables(bytes)?.into_iter();
    let mut merged = tables.next().expect("extract_tables never returns empty");
...
/// First row -> headers; remaining rows -> data, trimmed, with newlines folded
/// to spaces and every row padded/truncated to the header width.
fn normalise(table: Vec<Vec<Option<String>>>) -> ExtractedTable {
    ...
    let data = rows
        ...
        .collect();
    ExtractedTable { headers, rows: data }
}
...
mod tests {
    ...
    fn extracts_every_table_from_a_pdf() {
        let tables = extract_tables(include_bytes!("tests/fixtures/table.pdf")).unwrap();
    ...
    fn merges_all_tables_into_one() {
        let out = extract_table(include_bytes!("tests/fixtures/table.pdf")).unwrap();
        ...
        assert_eq!(out.rows[3], vec!["04/04/2021", "Fuel", "300.00"]);
