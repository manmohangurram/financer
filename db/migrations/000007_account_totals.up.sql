...
UPDATE accounts SET
  total_credit = (SELECT COALESCE(SUM(amount), 0) FROM transactions t WHERE t.account_id = accounts.id AND t.type = 'CREDIT'),
  total_debit  = (SELECT COALESCE(SUM(amount), 0) FROM transactions t WHERE t.account_id = accounts.id AND t.type = 'DEBIT');
